...
static void clickCallback(int event, int x, int y, int flags, void* param)
{
    ...
    if(event == cv::EVENT_LBUTTONDOWN && right_click==1) //eyedropper function
    {
        ...
        b = imageIn.at<cv::Vec3b>(y, x)[0];
        g = imageIn.at<cv::Vec3b>(y, x)[1];
        r = imageIn.at<cv::Vec3b>(y, x)[2];
        //pixel = imageIn.at<cv::Vec3b>(y, x);
        std::cout<<"COLOR UPDATED TO: ["<<b<<","<<g<<","<<r<<"]"<<std::endl;
    }

    else if(right_click==2)//crop function
    {
    ...
    else if(right_click==3)//Pencil Function
    {
    ...
    else if(event == cv::EVENT_LBUTTONDOWN && right_click==4)//Paintbucket function
    {
        cv::Rect area;
    ...
    else if(event == cv::EVENT_LBUTTONDOWN && right_click==0)//Reset Function
    {
        cv::destroyWindow("New");//if new is not present, cannot reset properly
        clonedImage=imageIn.clone();

        cv::imshow("imageIn", clonedImage);
        cv::waitKey();
    }
